...
func ConvertsV2Ray(buf []byte) ([]map[string]any, error) {
	...
	for _, line := range arr {
		...
		case "naive", "naive+https", "naive+quic":
			naive, err := parseNaiveShareLink(line, scheme, names)
			if err != nil {
				continue
			}

			proxies = append(proxies, naive)
		...
		case "vless":
			urlVLess, err := parseVShareURL(line)
			if err != nil {
				continue
			}
		...
		case "vmess":
			...
			if err != nil {
				...
				urlVMess, err := parseVShareURL(line)
				if err != nil {
					continue
				}
			...
			extra := parseVShareExtraAny(values["extra"])
			...
			} else if extra.serverName != "" {
				vmess["servername"] = extra.serverName
			}

			if realityPublicKey := firstNonEmpty(valueToString(values["pbk"]), extra.realityPublicKey); realityPublicKey != "" {
				vmess["reality-opts"] = map[string]any{
					"public-key": realityPublicKey,
					"short-id":   firstNonEmpty(valueToString(values["sid"]), extra.realityShortID),
				}
			}

			network, ok := values["net"].(string)
			useXHTTP := false
			useHTTPUpgrade := false
			if !ok || strings.TrimSpace(network) == "" {
				network = extra.downloadNetwork
				ok = network != ""
			}
			if ok {
			...
			if ok || extra.downloadSecurity != "" {
				if !ok {
					tls = extra.downloadSecurity
				}
				...
				if alpn, ok := values["alpn"].(string); ok {
					...
					vmess["alpn"] = extra.alpn
				}
				if extra.allowInsecure != nil {
					vmess["skip-cert-verify"] = *extra.allowInsecure
				}
			}

			host := firstNonEmpty(valueToString(values["host"]), extra.host, extra.downloadAddress)
			path := firstNonEmpty(valueToString(values["path"]), extra.path)
			mode := firstNonEmpty(valueToString(values["mode"]), extra.mode)
			var extraMap map[string]any
			switch raw := values["extra"].(type) {
			case map[string]any:
				extraMap = raw
			case string:
				extraMap = decodeVShareExtraMap(raw)
			}
