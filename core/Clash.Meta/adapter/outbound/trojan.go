...
func (t *Trojan) StreamConnContext(ctx context.Context, c net.Conn, metadata *C.Metadata) (_ net.Conn, err error) {
	...
	case "xhttp":
		...
		xhttpHost := firstNonEmptyString(firstString(t.option.XHTTPOpts.Host), t.option.SNI)
		xhttpPath := firstString(t.option.XHTTPOpts.Path)
		if xhttpPath == "" {
			xhttpPath = "/"
		}
		...
		headers := http.Header{}
		for key, rawValue := range t.option.XHTTPOpts.Headers {
...
func NewTrojan(option TrojanOption) (*Trojan, error) {
	...
	applyXHTTPHintsToTrojan(&option, xhttpHints)
