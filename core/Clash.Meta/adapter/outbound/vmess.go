...
type XHTTPOptions struct {
	Host    any            `proxy:"host,omitempty"`
	Path    any            `proxy:"path,omitempty"`
	Mode    string         `proxy:"mode,omitempty"`
	Headers map[string]any `proxy:"headers,omitempty"`
	Extra   map[string]any `proxy:"extra,omitempty"`
}
...
func (v *Vmess) StreamConnContext(ctx context.Context, c net.Conn, metadata *C.Metadata) (_ net.Conn, err error) {
	...
	case "xhttp":
		...
		xhttpHost := firstNonEmptyString(firstString(v.option.XHTTPOpts.Host), v.option.ServerName, host)
		xhttpPath := firstString(v.option.XHTTPOpts.Path)
		if xhttpPath == "" {
			xhttpPath = "/"
		}

		if !strings.HasSuffix(xhttpPath, "/") {
			xhttpPath += "/"
		}

		headers := http.Header{}
		for key, rawValue := range v.option.XHTTPOpts.Headers {
...
func NewVmess(option VmessOption) (*Vmess, error) {
	xhttpHints := extractXHTTPHints(&option.XHTTPOpts)
	option.Network = normalizeWSNetwork(option.Network, &option.WSOpts, &option.XHTTPOpts)
	applyXHTTPHintsToVmess(&option, xhttpHints)
