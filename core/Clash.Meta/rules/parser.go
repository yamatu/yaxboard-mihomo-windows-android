...
func ParseRule(tp, payload, target string, params []string, subRules map[string][]C.Rule) (parsed C.Rule, parseErr error) {
	switch tp {
	case "DOMAIN":
		parsed = RC.NewDomain(normalizeDomainLikePayload(payload), target)
	case "DOMAIN-SUFFIX":
		parsed = RC.NewDomainSuffix(normalizeDomainLikePayload(payload), target)
	case "DOMAIN-KEYWORD":
		parsed = RC.NewDomainKeyword(normalizeDomainLikePayload(payload), target)
	case "DOMAIN-REGEX":
...
func normalizeDomainLikePayload(payload string) string {
	normalized := strings.TrimSpace(strings.Trim(payload, "\"'"))
	if normalized == "" {
		return normalized
	}

	if host := tryExtractHostname(normalized); host != "" {
		normalized = host
	}
...
func tryExtractHostname(raw string) string {
