...
func (i *ClientInstance) Handshake(conn net.Conn) (*CommonConn, error) {
	...
	if i.Seconds > 0 {
		...
		if time.Now().Before(i.Expire) {
			...
			i.RWLock.RUnlock()
			c.PreWrite = clientHello[:ivAndRelaysLength+50]
			c.AEAD = NewAEAD(clientHello[ivAndRelaysLength+18:ivAndRelaysLength+50], c.UnitedKey, c.UseAES)
			if i.XorMode == 2 {
	...
	for idx, l := range paddingLens {
		...
		if len(paddingGaps) > idx {
			time.Sleep(paddingGaps[idx])
		}
	}

	encryptedPfsPublicKey := make([]byte, 1136)
	...
	peerX25519PKey, err := ecdh.X25519().NewPublicKey(encryptedPfsPublicKey[1088:1120])
	if err != nil {
		return nil, err
	}
	...
	c.PeerAEAD = NewAEAD(encryptedPfsPublicKey[:1120], c.UnitedKey, c.UseAES)
