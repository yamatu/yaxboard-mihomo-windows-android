...
final _logger = FileLogger('doh_dns_resolver.dart');
...
class DohDnsResolver {
    ...
    client.connectionFactory =
      ...
      if (!_isIpLiteral(normalizedHost)) {
        ...
        try {
          ...
          _logger.warning('[DoH] Resolve failed for $normalizedHost: $e');
        }

        if (resolvedIp != null) {
          connectHost = resolvedIp;
          usedDohIp = true;
          _logger.info('[DoH] Resolved $normalizedHost -> $resolvedIp');
        }
      }
  ...
  static Future<ConnectionTask<Socket>> _startConnectWithFallback({
    ...
    try {
      ...
      _logger.warning(
        '[DoH] Connect failed via resolved IP for '
        '$requestHost -> $primaryHost:$port, fallback to system DNS: $fallback:$port, error: $e',
      );
  ...
  static Future<_DnsCacheEntry?> _lookupRecord(
    ...
    try {
      ...
      _logger.warning('[DoH] Query $host ($type) failed: $e');
      return null;
    } finally {
